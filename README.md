...
- **YouTube 링크 맥락 분석**: 번역 전에 영상 스냅샷 5장을 추출해 모델이 장면을 이해하도록 보조
...
- YouTube 링크를 입력하면 번역 시작 전에 스냅샷을 기반으로 영상 요약이 생성되어 번역 프롬프트에 자동 반영됩니다.
- **Thinking Budget 설정**:
...
- 컨텍스트 압축 기능을 사용해도 업로드한 이미지 참조는 자동으로 유지되어 장면 정보가 사라지지 않습니다.
